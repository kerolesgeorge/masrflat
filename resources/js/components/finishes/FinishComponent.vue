<template>
    <div>
        ...
        <button class="btn btn-success mb-3" data-toggle="modal" data-target="#createFinish" @click="clearErrors">اضافة نوع جديد</button>
...
export default {
    ...
    methods: {
        ...
        fetchFinishes() {
            axios.get('/api/finishes').then(response => {
        ...
        createFinish(finish) {
            axios.post('/api/finishes/', finish).then(response => {
                this.fetchFinishes();
        ...
        // Delete finish
        deleteFinish() {
            axios.delete(`/api/finishes/${this.finishDeleteId}`).then(response => {
                this.fetchFinishes();
